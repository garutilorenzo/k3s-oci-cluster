...
if [[ "$first_instance" == "$instance_id" ]]; then
  ...
  first_last="first"
  until (curl -sfL https://get.k3s.io | K3S_TOKEN=${k3s_token} sh -s - --cluster-init $disable_traefik --node-ip $local_ip --advertise-address $local_ip --flannel-iface $flannel_iface --tls-san ${k3s_tls_san} --tls-san ${k3s_tls_san_public}); do
  ...
  echo ":( Cluster join"
  wait_lb
  until (curl -sfL https://get.k3s.io | K3S_TOKEN=${k3s_token} sh -s - --server https://${k3s_url}:6443 $disable_traefik --node-ip $local_ip --advertise-address $local_ip --flannel-iface $flannel_iface --tls-san ${k3s_tls_san} --tls-san ${k3s_tls_san_public}); do
    echo 'k3s did not install correctly'
    sleep 2
  done
fi
